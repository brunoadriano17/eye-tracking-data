    ...
    def detect_faces(self, frame):
        ...
        if ('last_face_detect_index' not in previous_frame or
            ...
            if detector.__class__.__name__ == 'CascadeClassifier':
                ...
                detections = detector(cv.resize(frame['grey'], (0, 0), fx=0.25, fy=0.25), 0)
                # rows, cols = frame['grey'].shape
                # detections = detector(cv.pyrDown(frame['grey'], dstsize=(cols//4, rows//4)))
            faces = []
            for d in detections:
                try:
