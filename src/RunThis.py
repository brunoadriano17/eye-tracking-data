...
currentI = 0
vetFinal = []
# vetTargets = [[20, 20], [960, 20], [1900, 20], [1900, 540], [960, 540], [20, 540], [20, 1060], [960, 1060], [1900, 1060]]
...
while run:
    ...
    for x in range(60):
        ...
        if ret:
            ...
            cv.imwrite('./out/out'+str(k)+'.jpg', frame)
            k = k+1
            count += 1
        else:
            x -= 1
...
#Output file for plot and data
path, dirs, files = next(os.walk("./calib_out"))
file_count = len(files)
fileName = './calib_out/' + str(file_count + 1)
mediaFileName = './calib_out/data/media_output/' + str(file_count + 1)
outTextFileName = './calib_out/data/' + str(file_count + 1)
...
#     # Carrega vetor para classe
#     ds = Video(outputVet,
#                         tensorflow_session=ses, batch_size=batch_size,
#                         data_format='NCHW' if gpu_available else 'NHWC',
#                         eye_image_shape=(36, 60))
#     # Modelo
#     model = ELG(
#         ses, train_data={'videostream': ds},
#         first_layer_stride=1,
#         num_modules=2,
#         num_feature_maps=32,
#         learning_schedule=[
